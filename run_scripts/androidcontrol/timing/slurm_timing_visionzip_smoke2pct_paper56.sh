#!/usr/bin/env bash
# VisionZip smoke2pct with paper-aligned 56% retention (dominant + contextual).
set -Eeuo pipefail
...
exec "${_SCRIPT_DIR}/_exec_timing_eval.sh" eval_visionzip_slurm.sh \
  --job-name timing-vz-smoke2pct-p56 \
  --config "${REPO_ROOT}/configs/androidcontrol/visionzip/smoke2pct_paper56.json" \
  --output-dir "${REPO_ROOT}/outputs/timing_visionzip_smoke2pct_paper56" \
  --time 04:00:00 \
  --measure-e2e-latency \
  "$@"
